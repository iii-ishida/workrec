...
export async function fetchTaskList(sessionCookie: string, limit: number, cursor: string = ""): Promise<TaskListItem[]> {
  const query = `
    ...
    .then(result => {
      ...
      return result.data.tasks.edges.map((edge: any) => {
        ...
        return {
          ...
          state: node.state,
          lastStartTime: node.lastWork ? new Date(node.lastWork.startTime) : null
        };
      });
    });
}

export async function createTask(sessionCookie: string, title: string): Promise<void> {
  const query = `
