...
RSpec.describe App::Models::WorkList do
...
RSpec.describe App::Models::WorkListItem do
  describe '#apply_events' do
    let(:user_id) { 'some-user-id' }
    let(:work_id) { 'some-work-id' }
    let(:title) { 'some-title' }

    let(:create_work_event) { App::Models::Event.new(id: 'event-01', user_id: user_id, work_id: work_id, action: 'create_work', title: title, time: nil, created_at: Time.now) }
    let(:start_work_event) { App::Models::Event.new(id: 'event-02', user_id: user_id, work_id: work_id, action: 'start_work', title: '', time: Time.now + 1 * 60, created_at: Time.now + 1) }
    let(:pause_work_event) { App::Models::Event.new(id: 'event-03', user_id: user_id, work_id: work_id, action: 'pause_work', title: '', time: Time.now + 2 * 60, created_at: Time.now + 2) }
    let(:resume_work_event) { App::Models::Event.new(id: 'event-04', user_id: user_id, work_id: work_id, action: 'resume_work', title: '', time: Time.now + 3 * 60, created_at: Time.now + 3) }

    let(:created_work) do
      App::Models::WorkListItem.new(
        id: work_id,
        user_id: user_id,
    ...
    context 'create work' do
      it 'returns the applied WorkListItem' do
        Timecop.freeze(Time.now) do
          expect(App::Models::WorkListItem::EMPTY.apply_events([create_work_event])).to eq(created_work)
        end
      end
    end
